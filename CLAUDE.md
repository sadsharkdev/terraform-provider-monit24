...
- Optional/nullable API fields are modeled as pointers (`*string`, `*bool`, `*[]int`, etc.) in the `client` structs, only set in `Read*` when non-nil — **except** an `Optional` non-`Computed` collection field (see `assigned_sensor_ids` on `monit24_group`, `sensor_ids`/`step_names` on `monit24_service`): there, `Read*` must call `d.Set` unconditionally, falling back to an empty slice/map when the API field is nil, since nil genuinely means "cleared" and skipping `d.Set` would leave a stale, previously-set value stuck in state forever if the API responds with null/omits the key once cleared.
- Plain `Create` delegates straight to `Read` after `SetId` in most resources, including `monit24_service` (it previously delegated to `Update` instead, but `Create` already sends every field the API accepts via `newServiceFromResourceData` before the POST, so the extra GET→rebuild→PUT→GET Update would have performed was pure waste). This is not universal, though: `monit24_user_data` and `monit24_user_data_setting` `return diags` directly without calling Read, since their schemas have no `Computed` fields for a Read to populate. Only delegate Create to Update if Update genuinely has information Create's POST doesn't, and never if Update has side effects beyond a plain PUT — see `monit24_subaccount`, which deliberately does not delegate Create to Update because Update also conditionally calls the `change_password` action. **`monit24_notification_address` also `return diags` directly without calling Read, but that's inconsistent with its own schema** — `group_id` is `Optional`+`Computed` there, so skipping Read leaves it unpopulated in state right after create until the next refresh; this predates this branch (untouched, byte-identical to `main`) and is a pre-existing gap, not something introduced here — worth fixing in a follow-up, not blocking.
...
- `monit24_subaccount` and `monit24_account_user` share their schema (`accountSchemaFields`/`userDataSchemaFields`), Read logic (`setAccountFields`), and `DeleteContext` (`resourceDeleteAccount`) — all in `monit24/resource_subaccount.go`, alongside `resourceAccountUser()` in `monit24/resource_account_user.go` which composes them with its own Create/Update/Read wrappers. `updateAccountAndPassword` is shared too: it only calls `UpdateAccount` when `d.HasChangesExcept("password")` is true, and routes a `password` change through `ChangeAccountPassword` separately — avoids a redundant PUT on password-only rotations and keeps the two resources' Update logic from drifting apart.
- There's no API operation to reset an account's password to empty, so clearing a previously-set `password` is rejected via `CustomizeDiff: rejectPasswordClear` on both `monit24_subaccount` and `monit24_account_user`, not inside `UpdateContext`: terraform-plugin-sdk/v2 persists the proposed diff into state even when an `Update` function returns an error (see the SDK's `ResourceData.Partial` doc comment), so rejecting inside Update would still leave state believing the clear succeeded — only rejecting at plan time, before `Apply` ever runs, actually prevents it.
...
Eleven resources are registered in `monit24/provider.go`, tracking Monit24 API v3.51:
